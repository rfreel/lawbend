#!/usr/bin/env python3
"""Compare public Keccak-256 hashing in stock Bend, native Lean, optimized C, and compact portable C."""
import hashlib,json,os,platform,statistics,subprocess,time
from pathlib import Path
from Crypto.Hash import keccak
P=Path(__file__).resolve().parents[1]
bins={'bend':P/'build/bench','lean':P/'build/lean/.lake/build/bin/keccakBench','c_optimized':P/'build/xkcp','c_portable':P/'build/xkcp-compact'}
...
def run(name,size,count):
    ...
    if name!='bend':assert out[2]==digest.hex(),(name,size,out)
...
for name in ['c_optimized','c_portable']:
    for n in list(range(274))+[4096,65536]:run(name,n,1)
print('Full digests: Lean 276, Bend 278; each C reference 276 passed',flush=True)
record={'host':platform.platform(),'cpu':subprocess.check_output(['sysctl','-n','machdep.cpu.brand_string'],text=True).strip(),
 ...
 'c_implementations':{'c_optimized':'XKCP plain-64bits fully unrolled; portable C, no assembly','c_portable':'XKCP compact64; portable C, no assembly'},
 ...
 'validation':{'lean_full_digests':276,'bend_cases':278,'c_optimized_full_digests':276,'c_portable_full_digests':276,'timed_checksums':'all samples checked against PyCryptodome Keccak'},
...
for size in [0,32,64,135,136,137,1024,16384,65536,1048576]:
    ...
    row={'bytes':size,'counts':counts,'samples_ms':samples,'us_per_hash':us,'lean_over_bend':us['lean']/us['bend'],'bend_over_c':us['bend']/us['c_optimized']}
    record['rows'].append(row)
    (P/'benchmarks/comparison-arm64.json').write_text(json.dumps(record,indent=2)+'\n')
